(function () {
  ...
  const myDropdown = function () {
    ...
    const onekit_submenu = function onekit_submenu() {
      ...
      var _loop = function _loop(i) {
        onekit_toggle[i].addEventListener("click", function (event) {
          ...
          var menu = onekit_toggle[i].nextElementSibling;
          var parentItem = onekit_toggle[i].closest("li");
          var willOpen = !menu.classList.contains("show");

          for (var j = 0; j < onekit_toggle.length; j++) {
            if (j !== i && onekit_toggle[j].nextElementSibling) {
              onekit_toggle[j].nextElementSibling.classList.remove("show");
              var otherParent = onekit_toggle[j].closest("li");
              if (otherParent) {
                otherParent.classList.remove("md-open");
              }
            }
          }
          ...
          if (parentItem) {
            parentItem.classList.toggle("md-open", willOpen);
          }
         });
        window.addEventListener("mouseup", function (event) {
          var menu = onekit_toggle[i].nextElementSibling;
          var toggleContains = onekit_toggle[i].contains(event.target);
          var menuContains = menu && menu.contains(event.target);

          if (!toggleContains && !menuContains) {
            if (menu) {
              menu.classList.remove("show");
            }
            var parentItem = onekit_toggle[i].closest("li");
            if (parentItem) {
              parentItem.classList.remove("md-open");
            }
          }
        });
      };
    ...
    var ef = document.querySelector(".dropdown-toggle");
    if ( ef != null) {
      onekit_submenu();
    }
  ...
  const myFooterConfig = function () {
    const headings = document.querySelectorAll('#footer-content h4');
    if (headings == null || !headings.length) {
      return;
    }

    const inDocs = window.location.pathname.indexOf('/docs/') > -1;
    const resolveHref = function (path) {
      if (!inDocs) {
        return path;
      }
    ...
    headings.forEach(function (heading) {
      ...
      list.innerHTML = links.map(function (item) {
        return '<li class="py-1 hover:text-white"><a href="' + resolveHref(item.href) + '">' + item.label + '</a></li>';
      }).join('');
    });

    const copyright = document.querySelector('.footer-dark .d-block.my-3');
    if (copyright) {
      copyright.innerHTML = '&copy; 2026 Meridian Data. All rights reserved.';
      copyright.classList.add('md-footer-copyright');
    }

    const footerBrandTitle = document.querySelector('#footer-content .flex.items-center span');
    if (footerBrandTitle) {
      footerBrandTitle.innerHTML = 'Meridian Data<span class="md-brand-tm">&trade;</span>';
      footerBrandTitle.classList.add('md-footer-brand-title');
    }

    const headerBrandTm = document.querySelector('body > header .mx-w-10 .text-xs');
    if (headerBrandTm) {
      headerBrandTm.classList.add('md-brand-tm');
      headerBrandTm.classList.remove('align-top');
    }

    const footerBrandTm = document.querySelector('#footer-content .md-footer-brand-title .md-brand-tm');
    if (footerBrandTm) {
      footerBrandTm.classList.add('md-brand-tm');
    }

    const footerBrandCopy = document.querySelector('#footer-content .flex.items-center + p');
    if (footerBrandCopy) {
